                ...
                <form method="POST" action="{{ route('admin.cards.update', [$card->id]) }}"
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('bank_of_card') ? 'is-invalid' : '' }}" name="bank_of_card"
                            ...
                            @foreach (App\Models\Card::BANK_SELECT as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('bank_of_card', $card->bank_of_card) === (string) $key ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('name_of_card') ? 'is-invalid' : '' }}" type="text"
                            name="name_of_card" id="name_of_card" value="{{ old('name_of_card', $card->name_of_card) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('card_number') ? 'is-invalid' : '' }}" type="text"
                            name="card_number" id="card_number" value="{{ old('card_number', $card->card_number) }}"  maxlength="16">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('expired_date') ? 'is-invalid' : '' }}" type="month"
                            name="expired_date" id="expired_date" value="{{ old('expired_date', $card->expired_date) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('cvv') ? 'is-invalid' : '' }}" type="text"
                            name="cvv" id="cvv" value="{{ old('cvv', $card->cvv) }}" maxlength="3">
                        @if ($errors->has('cvv'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($users as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('user_id') ? old('user_id') : $card->user->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('user'))
