    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.merchants.update", [$merchant->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $merchant->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $merchant->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contact_number') ? 'is-invalid' : '' }}" type="text" name="contact_number" id="contact_number" value="{{ old('contact_number', $merchant->contact_number) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Merchant::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $merchant->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" type="text" name="address" id="address" value="{{ old('address', $merchant->address) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('state') ? 'is-invalid' : '' }}" name="state_id" id="state_id">
                    @foreach($states as $id => $entry)
                        <option value="{{ $id }}" {{ (old('state_id') ? old('state_id') : $merchant->state->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('state'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('longitude') ? 'is-invalid' : '' }}" type="text" name="longitude" id="longitude" value="{{ old('longitude', $merchant->longitude) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('latitude') ? 'is-invalid' : '' }}" type="text" name="latitude" id="latitude" value="{{ old('latitude', $merchant->latitude) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ssm_number') ? 'is-invalid' : '' }}" type="text" name="ssm_number" id="ssm_number" value="{{ old('ssm_number', $merchant->ssm_number) }}">
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('ssm_document') ? 'is-invalid' : '' }}" id="ssm_document-dropzone">
                </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $merchant->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
