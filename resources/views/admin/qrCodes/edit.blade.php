...
@section('content')
    ...
    <div style="display: flex; justify-content: center; align-items: center;">
        <div class="card" style="width: 80%;">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.qr-codes.update', [$qrCode->id]) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code"
                            id="code" value="{{ old('code', $qrCode->code) }}">
                        @if ($errors->has('code'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                            ...
                            @foreach (App\Models\QrCode::STATUS_SELECT as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('status', $qrCode->status) === (string) $key ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control datetime {{ $errors->has('expired_at') ? 'is-invalid' : '' }}"
                            type="text" name="expired_at" id="expired_at"
                            value="{{ old('expired_at', $qrCode->expired_at) }}">
