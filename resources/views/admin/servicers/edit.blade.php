...
@section('content')
    ...
    <div style="display: flex; justify-content: center; align-items: center;">
        <div class="card" style="width: 80%;">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.servicers.update', [$servicer->id]) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                            id="name" value="{{ old('name', $servicer->name) }}">
                        @if ($errors->has('name'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id"
                            ...
                            @foreach ($users as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('user_id') ? old('user_id') : $servicer->user->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('user'))
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('merchant') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($merchants as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('merchant_id') ? old('merchant_id') : $servicer->merchant->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
