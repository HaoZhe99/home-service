            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Servicer">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('cruds.servicer.fields.user') }}
                        </th>
                        <th>
                ...
                <tbody>
                    @foreach($servicers as $key => $servicer)
                        <tr data-entry-id="{{ $servicer->id }}">
                            ...
                            <td>
                                @can('servicer_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.servicers.show', $servicer->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('servicer_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.servicers.edit', $servicer->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('servicer_delete')
                                    <form action="{{ route('admin.servicers.destroy', $servicer->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-Servicer:not(.ajaxTable)').DataTable({ buttons: dtButtons })
