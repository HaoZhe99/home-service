        ...
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('verify') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="verify" id="verify" value="1" {{ $user->verify || old('verify', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('verify_token') ? 'is-invalid' : '' }}" type="text" name="verify_token" id="verify_token" value="{{ old('verify_token', $user->verify_token) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('addresses') ? 'is-invalid' : '' }}" name="addresses[]" id="addresses" multiple>
                    @foreach($addresses as $id => $address)
                        <option value="{{ $id }}" {{ (in_array($id, old('addresses', [])) || $user->addresses->contains($id)) ? 'selected' : '' }}>{{ $address }}</option>
                    @endforeach
                </select>
